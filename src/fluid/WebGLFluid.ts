...
import {
  VERT, ADVECT, DIVERGENCE, JACOBI, GRADIENT_SUBTRACT,
  VORTICITY, VORT_CONFINE, BOUNDARY, SPLAT, DISPLAY,
} from './shaders';
...
function link(gl: WebGLRenderingContext, vertSrc: string, fragSrc: string): WebGLProgram {
...
const VELOCITY_DISS = 0.992;  // keep momentum so dye keeps sloshing
const DYE_DISS      = 0.990;  // colours fade over ~1s so they stay distinct
const VORT_STRENGTH = 6.0;    // gentle swirl — high values turn slosh into foam
const SPLAT_RADIUS  = 0.012;  // small soft blobs (reference-style)
...
const MAX_DYE_ENERGY   = 5.0; // soft cap — pause dye (not velocity) when saturated
const DYE_SPLAT_ENERGY = 1.0;
...
export class WebGLFluid {
  ...
  private readonly pBoundary: WebGLProgram;
  ...
  private dyeEnergy       = 0;   // tracks accumulated dye level; decays each frame
  ...
  constructor(gl: WebGLRenderingContext) {
    ...
    if (isWebGL2) {
      ...
      if (!gl.getExtension('EXT_color_buffer_float')) {
        throw new Error('Float framebuffer not supported (EXT_color_buffer_float missing)');
      }
    ...
    this.pBoundary = link(gl, VERT, BOUNDARY);
    ...
    [
      ...
      this.divergence,     this.vorticity,
    ].forEach(fbo => {
  ...
  setColors(weights: ColorWeight[]): void {
    ...
    if (total <= 0) return;
    this.colors = weights.map(w => ({ color: w.color, weight: w.weight / total }));
    this.pendingDyeClear = true;
    this.dyeEnergy = 0;
  }
  ...
  render(timeMs: number): void {
    ...
    // ── Decay energy estimate proportionally to dye dissipation ───────────
    this.dyeEnergy *= Math.pow(DYE_DISS, dt * 60);
    ...
    this.applyBoundary(this.velocity, 0);
  ...
  destroy(): void {
    const { gl } = this;
    [this.pAdvect, this.pDiv, this.pJacobi, this.pGradSub,
     this.pVort, this.pVortConf, this.pBoundary, this.pSplat, this.pDisplay
    ...
    [this.divergence, this.vorticity].forEach(s => {
      gl.deleteTexture(s.tex); gl.deleteFramebuffer(s.fbo);
    });
  }
  ...
  private makeTex(filter: number): WebGLTexture {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, this.floatInternal, SIM_W, SIM_H, 0,
      gl.RGBA, this.floatType, null);
  ...
  private makeDouble(): DoubleFBO {
    ...
    return { read, write, swap() { const t = this.read; this.read = this.write; this.write = t; } };
  }
  ...
  private advectVelocity(dt: number): void {
    const { gl } = this;
    const p = this.pAdvect; this.use(p);
  ...
  private solvePressure(): void {
    const { gl } = this;
    const p = this.pJacobi; this.use(p);
    gl.uniform2fv(gl.getUniformLocation(p, 'u_texelSize'), TEXEL_SIZE);
  ...
  private subtractGradient(): void {
    const { gl } = this;
    const p = this.pGradSub; this.use(p);
    this.bindTex(p, 'u_velocity', this.velocity.read.tex, 0);
  ...
  private applyVortConfinement(dt: number): void {
    const { gl } = this;
    const p = this.pVortConf; this.use(p);
    this.bindTex(p, 'u_velocity',  this.velocity.read.tex, 0);
  ...
  /** Zero field values outside Austria (SDF-based wall). */
  private applyBoundary(dfbo: DoubleFBO, _scale: number): void {
    const { gl } = this;
    const p = this.pBoundary; this.use(p);
    this.bindTex(p, 'u_field', dfbo.read.tex, 0);
    this.bindTex(p, 'u_sdf',   this.sdfTex,   1);
    gl.uniform1f(gl.getUniformLocation(p, 'u_scale'), 0);
  ...
  private drawDisplay(w: number, h: number): void {
    const { gl } = this;
    const p = this.pDisplay; this.use(p);
   ...
   * Velocity is always injected (keeps the fluid moving); dye is paused when the
   * basin is saturated so colours don't pile up to white.
   */
  private addSplat(): void {
    ...
    this.splatField(this.velocity, u, v, SPLAT_RADIUS, [dx, dy, 0, 0],
      -1e9, 1e9, 0);

    if (this.dyeEnergy < MAX_DYE_ENERGY) {
      const c = this.sampleColor();
      // dye: PAINT the pure fuel colour (mix toward it), clamp [0,1].
      // Strength multiplier keeps each splat slightly translucent so it blends
      // with whatever colour is already there instead of hard-stamping.
      const s = DYE_STRENGTH;
      this.splatField(this.dye, u, v, SPLAT_RADIUS, [c[0] * s, c[1] * s, c[2] * s, s],
        0, 1, 1);
      this.dyeEnergy += DYE_SPLAT_ENERGY;
    }
  }
  ...
  private splatField(
    ...
    const { gl } = this;
    gl.viewport(0, 0, SIM_W, SIM_H);
    const p = this.pSplat; this.use(p);
